...
public class GreetChild2Behavior extends AbstractBehavior<RespondMessage> {
    ...
    public Receive<RespondMessage> createReceive() {
        return newReceiveBuilder().onMessage(RespondMessage.class, this::behave).build();
    }

    private Behavior<RespondMessage> behave(RespondMessage message) {
        greetingCounter++;
        getContext().getLog().info("Respond {} from {}", greetingCounter, message.fromWhom);
        if (greetingCounter == max) {
            return Behaviors.stopped();
        } else {
            message.fromWhomActor.tell(new GreetMessage("child2", getContext().getSelf()));
            return this;
        }
    }
}
